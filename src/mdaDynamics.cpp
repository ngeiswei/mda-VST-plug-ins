...
float mdaDynamics::getParameter(VstInt32 index)
{
	float v=0;
...
void mdaDynamics::getParameterName(VstInt32 index, char *label)
{
	switch(index)
  {
...
void mdaDynamics::getParameterLabel(VstInt32 index, char *label)
{
	switch(index)
  {
...
void mdaDynamics::process(float **inputs, float **outputs, VstInt32 sampleFrames)
{
	float *in1 = inputs[0];
	float *in2 = inputs[1];
...
void mdaDynamics::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames)
{
	float *in1 = inputs[0];
	float *in2 = inputs[1];
	float *out1 = outputs[0];
