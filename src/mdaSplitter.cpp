//
// VST mda plug-in: "mda Splitter" v1.0
//
...
float mdaSplitter::getParameter(VstInt32 index) { return param[index]; }
...
void mdaSplitter::process(float **inputs, float **outputs, VstInt32 sampleFrames)
{
  float *in1 = inputs[0];
  float *in2 = inputs[1];
...
void mdaSplitter::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames)
{
  float *in1 = inputs[0];
  float *in2 = inputs[1];
  float *out1 = outputs[0];
